...
app.get("/api/xml-parser/latest", async (req, res) => {
  res.set("Content-Type", "application/json");
  const clientID = req.query.clientid;
  const secret = req.query.secret;
  const images = req.query.images || false;
  ...
  const ctx = await fetch(`https://www.growtopiagame.com/forums/external.php?type=RSS2&lastpost=1`).then(res => res.text());
  const json = JSON.parse(xmlParser.toJson(ctx));
  const payload = {
    code: 200,
    posts: []
